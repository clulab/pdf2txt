...
class LineWrapPreprocessorTest extends Test {
  ...
  test(
    "Through this process, we have also introduced several new PyTorch features such as the pad_sequence, pack_padded_sequence, and pad_packed_sequence functions, which allow us to work more e[ff]iciently with variable length sequences for recurrent neural networks."
    "Through this process, we have also introduced several new PyTorch features such as the pad_sequence, pack_padded_sequence, and pad_packed_sequence functions, which allow us to work more efficiently with variable length sequences for recurrent neural networks."
  )
